...
//	iprintlnbold( &"ZOMBIE_START_TPAD" );
	...
	{
		players[i] thread maps\factory\_zombiemode_timer::start_timer( time+1, "stop_countdown" );
	}
	wait( time+1 );
...
player_teleporting( index )
{
	...
	if ( IsDefined( ss ) )
	{
		ss thread maps\factory\_zombiemode_powerups::special_powerup_drop(ss.origin);
	}
	...
	if ( time_since_last_teleport < 60000 && level.active_links == 3 && level.round_number > 20 )
	{
		...
		maps\factory\_zombiemode_dogs::special_dog_spawn( undefined, 4 );
