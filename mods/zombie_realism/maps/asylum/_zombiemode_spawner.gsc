...
crash_into_building()
{
	...
	while( 1 )
	{
		...
		while( 1 )
		{
			...
			level thread maps\asylum\_zombiemode_blockers::remove_chunk( chunk, self.first_node, true );
	...
	while( 1 )
	{
		...
		if( notetrack == "board" )
		{
			if( !chunk.destroyed )
			{
				...
				level thread	maps\asylum\_zombiemode_blockers::remove_chunk( chunk, node,true );
			}
		}
	}
}
...
zombie_death_points( origin, mod, hit_location, player,zombie )
{
	...
	level thread maps\asylum\_zombiemode_powerups::powerup_drop( origin );
...
zombie_damage( mod, hit_location, hit_origin, player, is_dog )
{
	...
	self thread maps\asylum\_zombiemode_powerups::check_for_instakill( player );
}
