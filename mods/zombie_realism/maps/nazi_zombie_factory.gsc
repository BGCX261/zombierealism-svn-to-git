...
#include maps\factory\_zombiemode_zone_manager; 
...
//	level.crawlers_enabled = true;		//MM - added for crawler support
	...
	level.door_dialog_function = maps\factory\_zombiemode::play_door_dialog;
	level.achievement_notify_func = maps\factory\_zombiemode_utility::achievement_notify;
	level.dog_spawn_func = maps\factory\_zombiemode_dogs::dog_spawn_factory_logic;
	...
	maps\factory\_zombiemode::main("receiver_zone_spawners");
...
//	level thread setup_door_waits();
	...
	level thread maps\factory\_zombiemode_zone_manager::manage_zones( "receiver_zone" );
...
bridge_init()
{
	...
	maps\factory\_zombiemode_zone_manager::connect_zones( "wnuen_bridge_zone", "bridge_zone" );
	maps\factory\_zombiemode_zone_manager::connect_zones( "warehouse_top_zone", "bridge_zone" );
}
...
jump_from_bridge()
{
	...
	maps\factory\_zombiemode_zone_manager::connect_zones( "outside_south_zone", "bridge_zone", true );
	maps\factory\_zombiemode_zone_manager::connect_zones( "outside_south_zone", "wnuen_bridge_zone", true );
}


init_sounds()
{
	maps\factory\_zombiemode_utility::add_sound( "break_stone", "break_stone" );
	maps\factory\_zombiemode_utility::add_sound( "gate_door",	"open_door" );
	maps\factory\_zombiemode_utility::add_sound( "heavy_door",	"open_door" );
}
...
//	include_weapon( "type99_rifle_scoped_zombie" );	//
	...
	// limited weapons
	maps\factory\_zombiemode_weapons::add_limited_weapon( "zombie_colt", 0 );
	//maps\_zombiemode_weapons::add_limited_weapon( "zombie_type99_rifle", 0 );
	maps\factory\_zombiemode_weapons::add_limited_weapon( "zombie_gewehr43", 0 );
	maps\factory\_zombiemode_weapons::add_limited_weapon( "zombie_m1garand", 0 );
}
...
factory_cymbal_monkey_weighting_func()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		if( players[i] maps\factory\_zombiemode_weapons::has_weapon_or_upgrade( "zombie_cymbal_monkey" ) )
		{
			count++;
		}
	}
...
include_powerups()
{
	...
	include_powerup( "berserk" );
	include_powerup( "firedamage" );
}
...
//	clientnotify( "power_on" );
	...
	maps\factory\_zombiemode_zone_manager::connect_zones( "outside_east_zone", "outside_south_zone" );
	maps\factory\_zombiemode_zone_manager::connect_zones( "outside_west_zone", "outside_south_zone", true );
}
...
zapper_light_red( lightname )
{
	...
	for(i=0;i<zapper_lights.size;i++)
	{
		...
		zapper_lights[i].fx = maps\factory\_zombiemode_net::network_safe_spawn( "trap_light_red", 2, "script_model", zapper_lights[i].origin );
...
zapper_light_green( lightname )
{
	...
	for(i=0;i<zapper_lights.size;i++)
	{
		...
		zapper_lights[i].fx = maps\factory\_zombiemode_net::network_safe_spawn( "trap_light_green", 2, "script_model", zapper_lights[i].origin );
		zapper_lights[i].fx setmodel("tag_origin");
		zapper_lights[i].fx.angles = zapper_lights[i].angles+(-90,0,0);
...
flytrap()
{
	...
	while ( !upgrade_hit )
	{
		...
		if ( maps\factory\_zombiemode_weapons::is_weapon_upgraded( weapon ) )
		{
			upgrade_hit = true;
		}
	}
	...
	while(1)
	{
		...
		if( level.round_number <= round )
		{
			if( vox_rand <= 2 )
			{
				...
				index = maps\factory\_zombiemode_weapons::get_player_index(players[p]);
				plr = "plr_" + index + "_";
...
play_level_easteregg_vox( object )
{
	...
	while(1)
	{
		...
		if( vox_rand <= percent )
		{
			index = maps\factory\_zombiemode_weapons::get_player_index(who);
			plr = "plr_" + index + "_";
