...
main()
{
	...
	// Added by Rampage - other variables
	rampage_variables();
...
//	level.global_kill_func = maps\_zombiemode_spawner::zombie_death; 
	...
	//level thread maps\_rampage_drones::do_drones();
...
init_levelvars()
{
	...
	set_zombie_var( "zombie_health_increase_percent", 	15, 	100 );
	...
	set_zombie_var( "zombie_score_bonus_melee", 		5 );
	set_zombie_var( "zombie_score_bonus_head", 			5 );
	set_zombie_var( "zombie_score_bonus_neck", 			3 );
	set_zombie_var( "zombie_score_bonus_torso", 		2 );
	set_zombie_var( "zombie_score_bonus_burn", 			1 );
...
player_is_cheating()
{
	...
	if( getDvarInt( "g_speed" ) > 190 && !level.berserk )
	{
		level.cheating = true;
		level.cheating2 += 1;
	}
...
onPlayerConnect()
{
	for( ;; )
	{
		...
		player.firedamage = false;
	}
}

// Added by Rampage - for testing purposes :)
xp_thread()
{
	self endon ( "death" );
	self endon ( "disconnect" );
	...
	{
		wait ( 2 );
			self maps\_challenges_coop::giveRankXP( "kill", 1000 );
	}
}
...
//				self maps\_zombiemode_score::create_player_score_hud(); 
				...
				if(self.class_picked == "support_1")
				{
					self thread ammo_regen(self.rank);
				}
			}
		}
...
ammo_regen(rank)
{
	...
	weapon_max_ammo = weaponMaxAmmo(weapon);
	weapon_stock_ammo = self getWeaponAmmoStock(weapon);
	...
	while(1)
	{
		...
		if(given_ammo == 1)
		{
			if(weapon_stock_ammo > (weapon_max_ammo - 1))
			{
				continue;
			}
		}
		else if(given_ammo == 2)
		{
			if(weapon_stock_ammo > (weapon_max_ammo - 2))
			{
				continue;
			}
		}
		else if(given_ammo == 3)
		{
			if(weapon_stock_ammo > (weapon_max_ammo - 3))
			{
				continue;
			}
		}
		
		self setWeaponAmmoStock(weapon, weapon_stock_ammo+given_ammo);
...
round_start()
{
	level.zombie_health = randomIntRange(200,300); 
...
compile_bonus()
{
	...
	hardest2killNum = players[0].downs1;
	hardest2kill = [];
	hardest2kill[0] = 0; 	
	
	mostRevivesNum = players[0].revives1;
	mostRevives = [];
	mostRevives[0] = 0; //index into players
	
	mostHeadshotsNum = players[0].headshots1;
	mostHeadshots = [];
	mostHeadshots[0] = 0; 	

	mostKnivesNum = players[0].knives1;
	mostKnives = [];
	mostKnives[0] = 0; 
	
	for(i = 1; i < players.size; i++)
	{
		if(players[i].downs1 < hardest2killNum && hardest2killNum > 0)
		{
			hardest2killNum = players[i].downs1;
			hardest2kill = [];
			hardest2kill[0] = i; 
		}
		else if(players[i].downs1 == hardest2killNum)
		{
			hardest2kill[hardest2kill.size] = i; 
		}
	
		if(players[i].revives1 > mostRevivesNum && mostRevivesNum > 0)
		{
			mostRevivesNum = players[i].revives1;
			mostRevives = [];
			mostRevives[0] = i; 
		}
		else if(players[i].revives1 == mostRevivesNum)
		{
			mostRevives[mostRevives.size] = i;
		}
		
		if(players[i].headshots1 > mostHeadshotsNum && mostHeadshotsNum > 0)
		{
			mostHeadshotsNum = players[i].headshots1;
			mostHeadshots = [];
			mostHeadshots[0] = i; 
		}
		else if(players[i].headshots1 == mostHeadshotsNum)
		{
			mostHeadshots[mostHeadshots.size] = i; 
		}		
		
		if(players[i].knives1 > mostKnivesNum && mostKnivesNum > 0)
		{
			mostKnivesNum = players[i].knives1;
			mostKnives = [];
			mostKnives[0] = i; 
		}
		else if(players[i].knives1 == mostKnivesNum)
		{
			mostKnives[mostKnives.size] = i; 
		}	
	}	
	
	if(hardest2killNum > 0)
	{
	...
	if(mostRevivesNum > 0) 
	{
		...
		{	
			if( players[ mostrevives[i] ].score > 0 && players[ mostheadshots[i] ].revives1 > 0 )
			{		
	...
	if(mostHeadshotsNum > 0) 
	{
		...
		{
			if( players[ mostheadshots[i] ].score > 0 && players[ mostheadshots[i] ].headshots1 > 0 )
			{		
	...
	if(mostKnivesNum > 0) 
	{
		for(i = 0; i < mostKnives.size; i++)
		{
			if( players[ mostknives[i] ].score > 0 && players[ mostknives[i] ].knives1 > 0 )
			{		
				players[ mostknives[i] ].mostknives = true;
			}
		}		
	}	
}
...
round_bonus()
{
	hud_bonus = newHudElem();
	hud_bonus.alignx = "left";
	hud_bonus.aligny = "top";
	hud_bonus.alpha = 1;
	hud_bonus.fadeovertime = 1;
	hud_bonus.color = ( 0.5, 1, 0.5 );
	size1 = 0;

	if( self.mostheadshots )
	{
		hud_bonus setText( self.playername+" was Top Headhunter with "+self.headshots1+" headshots" );
		size1 += 1;
	
		self maps\_zombiemode_score::add_to_player_score( 800 );
		self playSound( "cha_ching" );
	}
	
	if( self.mostknives )
	{
		hud_bonus setText( self.playername+" was Top Knife Killer with "+self.knives1+" knife kills" );
		size1 += 1;
	
		self maps\_zombiemode_score::add_to_player_score( 1000 );
		self playSound( "cha_ching" );
	}
	
	if( self.mostrevives )
	{
		hud_bonus setText( self.playername+" was Top Lifesaver with "+self.revives1+" revives" );
		size1 += 1;
	...
	{
		hud_bonus setText( self.playername+" was Hardest2Kill with "+self.downs1+" downs" );
		size1 += 1;
