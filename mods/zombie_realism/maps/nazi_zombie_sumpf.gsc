...
main()
{
	...
	maps\sumpf\_zombiemode_weapons_sumpf::add_zombie_weapon( "mine_bouncing_betty",&"ZOMBIE_WEAPON_SATCHEL_2000", 2000 );		
	maps\sumpf\_zombiemode_weapons_sumpf::add_zombie_weapon( "ptrs41_zombie", 						&"ZOMBIE_WEAPON_PTRS41_750", 				750,	"vox_sniper", 5);	
...
add_tesla_gun()
{
	while(1)
	{
		...
		{
			maps\sumpf\_zombiemode_weapons_sumpf::add_limited_weapon( "tesla_gun", 1);
			break;	
		}
	}
}
...
include_weapons()
{
	...
	// limited weapons
	maps\sumpf\_zombiemode_weapons_sumpf::add_limited_weapon( "zombie_colt", 0 );
	maps\sumpf\_zombiemode_weapons_sumpf::add_limited_weapon( "zombie_type99_rifle", 0 );
	maps\sumpf\_zombiemode_weapons_sumpf::add_limited_weapon( "zombie_gewehr43", 0 );
	maps\sumpf\_zombiemode_weapons_sumpf::add_limited_weapon( "zombie_m1garand", 0 );
...
{
	maps\sumpf\_zombiemode_weapons_sumpf::include_zombie_weapon( weapon_name );
}

include_powerup( powerup_name )
{
	maps\sumpf\_zombiemode_powerups::include_zombie_powerup( powerup_name );
}
...
spawn_initial_outside_zombies( name )
{
   ...
   for( i = 0; i < spawn_points.size; i++)
   {
		...
		if( IsDefined( ai ) )
		{
			ai maps\sumpf\_zombiemode_spawner::zombie_setup_attack_properties();
			ai thread maps\sumpf\_zombiemode_spawner::find_flesh();
			wait_network_frame();
		}
	}
}	
...
//	off_the_hook = spawn ("script_origin", toilet_trig.origin);
	...
	index = maps\_zombiemode_weapons_sumpf::get_player_index(player);
	player_index = "plr_" + index + "_";
	if(!IsDefined (self.vox_audio_secret))
	{
		num_variants = maps\sumpf\_zombiemode_spawner::get_number_variants(player_index + "vox_audio_secret");
	...
	player maps\sumpf\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, 0);
...
{	
	...
	{
		num_variants = maps\sumpf\_zombiemode_spawner::get_number_variants(player_index + "vox_oh_shit");
		self.vox_oh_shit = [];
		for(i=0;i<num_variants;i++)
		{
