...
init()
{
	...
	level.enemy_dog_spawns = getEntArray( "zombie_spawner", "script_noteworthy" ); 
	...
	level._effect[ "z_explode" ]			= Loadfx( "maps/zombie/fx_zombie_dog_explosion" );
}
...
watch_grenade_throw()
{
	...
	while(1)
	{
		...
		if(self.grenadeWeapon != "brain")
		{
			iPrintLn("no brain");
			continue;
		}
		
		self animscripts\combat_utility::TryGrenade(players[randomInt(players.size)],undefined);
		self ThrowGrenade();
		//MagicBullet(self.grenadeWeapon,releasepoint,players[randomInt(players.size)].origin);
...
{
	self waittill("death");
	self thread zombie_explode(self.origin);
}

zombie_explode(origin)
{
	fx = spawn("script_model",origin);
	fx setModel("tag_origin");
	playFXOnTag(level._effect["z_explode"], fx, "tag_origin");
	wait(5);
	fx delete();
}