...
zombie_devgui_give_powerup( powerup_name )
{
	...
	level thread maps\sumpf\_zombiemode_powerups::powerup_drop( trace["position"] );
}
...
zombie_devgui_goto_round( target_round )
{
	...
	{
		maps\sumpf\_zombiemode::ai_calculate_health();
		level.round_number++;
	}
	...
	{
		level.chalk_hud2 maps\sumpf\_zombiemode_utility::destroy_hud();

		if ( level.round_number < 11 )
		{
			level.chalk_hud2 = maps\sumpf\_zombiemode::create_chalk_hud( 64 );
		}
	}

	if ( IsDefined( level.chalk_hud1 ) ) 
	{
		level.chalk_hud1 maps\sumpf\_zombiemode_utility::destroy_hud();
		level.chalk_hud1 = maps\sumpf\_zombiemode::create_chalk_hud();
