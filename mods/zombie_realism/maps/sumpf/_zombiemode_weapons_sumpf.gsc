...
{	
	...
	while( 1 )
	{
		...
		{
			user thread maps\sumpf\_zombiemode_perks::play_no_money_perk_dialog();
			continue;	
		}
...
treasure_chest_weapon_spawn( chest, player )
{
	...
	for( i = 0; i < 40; i++ )
	{
		...
		/#
			if( maps\sumpf\_zombiemode_tesla::tesla_gun_exists() )	
			{
		...
		if( self.has_been_used_once )
		{
			...
			if( !player_has_weapon )
			{
				...
				else // not enough money
				{
					...
					player thread maps\sumpf\_zombiemode_perks::play_no_money_perk_dialog();
				}			
			}
...
//		iprintlnbold("Play_Weap_VO_" + weap);
		...
		self maps\sumpf\_zombiemode_spawner::do_player_playdialog(plr, sound_to_play, 0.05);
...
add_weapon_to_sound_array(vo,num)
{
	...
	for(i=0;i<player.size;i++)
	{
		...
		num = maps\sumpf\_zombiemode_spawner::get_number_variants(player_index + vo);
	}
//	iprintlnbold(vo);
