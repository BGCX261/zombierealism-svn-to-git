			...
			{				
				if(!isDefined(who.has_betties))
				{
					...
					{						
						...
						model thread maps\factory\_zombiemode_weapons::weapon_show( who ); 
...
betty_think()
{
	...
	for(i=0;i<zombs.size;i++)
	{
		...
		{
			zombs[i] thread maps\factory\_zombiemode_spawner::zombie_damage( "MOD_ZOMBIE_BETTY", "none", zombs[i].origin, self.owner );
		}
	}
