...
init()
{
	...
	else if( level.gameSkill2 == 2 )
	{
		...
		level.zombie_health = randomIntRange(300,400); 
	}
	else if( level.gameSkill2 == 3 )
	{
		level.zombie_move_speed = 10;
		level.zombie_health = randomIntRange(400,500); 
	}
...
crash_into_building()
{
	...
	while( 1 )
	{
		...
		while( 1 )
		{
			...
			level thread maps\factory\_zombiemode_blockers_new::remove_chunk( chunk, self.first_node, true );
	...
	while( 1 )
	{
		...
		if( notetrack == "board" )
		{
			if( !chunk.destroyed )
			{
				...
				level thread maps\factory\_zombiemode_blockers_new::remove_chunk( chunk, node, true );
			}
		}
	}
}
...
headshot_blood_fx()
{
	...
	if(IsDefined( self ))
	{
		if( self maps\factory\_zombiemode_tesla::enemy_killed_by_tesla() )
		{
...
zombie_death_points( origin, mod, hit_location, player,zombie )
{
	...
	{
		level thread maps\factory\_zombiemode_powerups::powerup_drop( origin );
	}
...
zombie_damage( mod, hit_location, hit_origin, player )
{
	...
	else if( self maps\factory\_zombiemode_tesla::is_tesla_damage( mod ) )
	{
		self maps\factory\_zombiemode_tesla::tesla_damage_init( hit_location, hit_origin, player );
		return;
	}
	else
	{
	...
	self thread maps\factory\_zombiemode_powerups::check_for_instakill( player );
}
...
play_closeDamage_dialog()
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_dmg_close");
		self.vox_dmg_close = [];
		for(i=0;i<num_variants;i++)
		{
...
do_zombie_rise()
{
	...
	// face goal
	target_org = maps\factory\_zombiemode_spawner::get_desired_origin();
