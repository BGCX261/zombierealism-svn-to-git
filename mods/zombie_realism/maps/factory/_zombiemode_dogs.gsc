...
#include maps\factory\_zombiemode_net;
#include maps\_music; 
...
dog_spawner_init()
{
	...
	{
		if( maps\factory\_zombiemode_spawner::is_spawner_targeted_by_blocker( dogs[i] ) )
		{
...
dog_round_aftermath()
{
	...
	if( IsDefined( power_up_origin ) )
	{
		...
		level thread maps\factory\_zombiemode_powerups::powerup_drop( power_up_origin );
...
add_dog_dialog(player)
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_dog_spawn");
	...
	player maps\factory\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, 0.25);
}
...
dog_init()
{
	...
	self thread maps\factory\_zombiemode::round_spawn_failsafe();
	self hide();
	...
	self maps\factory\_zombiemode_spawner::zombie_history( "zombie_dog_spawn_init -> Spawned = " + self.origin );
}
...
{
	self maps\factory\_zombiemode_spawner::zombie_history( "zombie_setup_attack_properties()" );
