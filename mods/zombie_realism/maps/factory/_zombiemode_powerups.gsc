...
//	add_zombie_powerup( "nuke", 		"zombie_bomb",		&"ZOMBIE_POWERUP_NUKE", 			"misc/fx_zombie_mini_nuke_hotness" );
	...
	// New
	add_zombie_powerup( "berserk",  	"zombie_brain",	&"ZOMBIE_POWERUP_MAX_AMMO");
...
powerup_drop(drop_point)
{
	...
	powerup = maps\factory\_zombiemode_net::network_safe_spawn( "powerup", 1, "script_model", drop_point + (0,0,40));
...
//	powerup = "dog";
	...
	case "dog":
		if ( level.round_number >= 15 )
		{
			...
			maps\factory\_zombiemode_dogs::special_dog_spawn( dog_spawners, 1 );
			...
			if (distance (players[i].origin, self.origin) < 64)
			{
				...
				else
				{
					...
					case "carpenter":
						...
						break;
					case "berserk":
							if(players[i].berserk)
							{
								return;
							}
							players[i].berserk = true;
							players[i] thread berserk_powerup();
							break;	
					case "firedamage":
							if(players[i].firedamage)
							{
								return;
							}
							players[i].firedamage = true;
							players[i] thread fire_powerup();
							break;	
...
start_carpenter( origin )
{
	...
	while(true)
	{
		...
		while(1)
		{
			...
			windows thread maps\factory\_zombiemode_blockers_new::replace_chunk( chunk, false, true );
...
check_for_instakill( player )
{
	if( IsDefined( player ) && IsAlive( player ) && level.zombie_vars["zombie_insta_kill"])
	{
		...
		{
			self maps\factory\_zombiemode_spawner::zombie_head_gib();
			self DoDamage( self.health + 666, self.origin, player );
			player notify("zombie_killed");
