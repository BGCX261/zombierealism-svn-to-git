...
vending_upgrade()
{
	...
	self thread maps\factory\_zombiemode_weapons::decide_hide_show_hint();
	...
	for( ;; )
	{
		...
		if( !player maps\factory\_zombiemode_weapons::can_buy_weapon() )
		{
			wait( 0.1 );
			continue;
		}
	...
	while( true )
	{
		...
		if( trigger_player == player ) 
		{
			if( !player maps\_laststand::player_is_in_laststand() )
			{
				...
				{
					player maps\factory\_zombiemode_weapons::weapon_give( weapon+"_upgraded" );
				}
				else
				{
...
play_no_money_perk_dialog()
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_nomoney_perk");
...
play_packa_wait_dialog(player_index)
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_perk_packa_wait");
...
play_packa_get_dialog(player_index)
{
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_perk_packa_get");
