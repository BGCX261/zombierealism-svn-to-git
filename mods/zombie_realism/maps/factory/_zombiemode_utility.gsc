...
generated_radius_attract_positions( forward, offset, num_positions, attract_radius )
{
	...
	for( i = offset; i < 360+offset; i += degs_per_pos )
	{
		...
		pos = maps\factory\_zombiemode_server_throttle::server_safe_ground_trace( "poi_trace", 10, self.origin + rotated_forward + ( 0, 0, 100 ) );
...
play_killstreak_dialog()
{
		...
		{
			num_variants = maps\factory\_zombiemode_spawner::get_number_variants(player_index + "vox_killstreak");
			self.vox_killstreak = [];
...
{              
	...
	{
		num_variants = maps\factory\_zombiemode_spawner::get_number_variants( player_index + dialog_category );                  
	...
	self maps\factory\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, waittime, response);
}
...
{              	
	...
	sound_to_play = dialog_category + "_" + variation;
	self maps\factory\_zombiemode_spawner::do_player_playdialog(player_index, sound_to_play, waittime);
}
...
include_weapon( weapon_name, in_box, weighting_func )
{
	...
	maps\factory\_zombiemode_weapons::include_zombie_weapon( weapon_name, in_box, weighting_func );
}

include_powerup( powerup_name )
{
	maps\factory\_zombiemode_powerups::include_zombie_powerup( powerup_name );
}

include_achievement( achievement, var1, var2, var3, var4 )
{
	maps\factory\_zombiemode_achievement::init( achievement, var1, var2, var3, var4 );
}
