...
blocker_trigger_think()
{
	...
	while( 1 )
	{
		...
		while( 1 )
		{
			...
			self thread replace_chunk( chunk, fast, false, sleight ); 
...
replace_chunk( chunk, eng, via_powerup, sleight )
{
	if( !IsDefined( eng ) )
	{
		eng = false;
	...
	if( eng == true )
	{
		chunk MoveTo( only_z, 0.1);
		chunk RotateTo( ( 0, 0, 0 ),  0.1 ); 
		chunk waittill_notify_or_timeout( "rotatedone", 1 ); 
		wait( 0.1 ); 
	}
	else if( sleight == true )
	{
		chunk MoveTo( only_z, 0.3); 
	...
	{
		chunk MoveTo( only_z, 0.4); 
		chunk RotateTo( ( 0, 0, 0 ),  0.4 ); 
		chunk waittill_notify_or_timeout( "rotatedone", 1 ); 
		wait( 0.4 ); 
	}
	...
	if( eng == true )
	{
		chunk MoveTo( chunk.og_origin, 0.2 ); 
	}
	else if( sleight == true )
	{
		chunk MoveTo( chunk.og_origin, 0.4 ); 
	}
	else
	{
		chunk MoveTo( chunk.og_origin, 0.5 ); 
	}
